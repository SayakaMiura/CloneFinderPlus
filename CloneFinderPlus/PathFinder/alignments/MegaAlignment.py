...
class MegaAlignment():
    ...
    def UpMeg(self, Name2Seq0, NameLs):
            ...
            return out 
    def remove_identical_clones(self,Clo2Seq,T2C2F,OutFileID):
        
        Done=[]
        Clo2NewName={}
        NewCloSeqDic={}		
        for Clo1 in Clo2Seq:		
            if Done.count(Clo1)==0:
                Done.append(Clo1)			
                Seq1=Clo2Seq[Clo1]
                Combo=[Clo1.replace('#','')]				
                for Clo2 in Clo2Seq:
                    if Done.count(Clo2)==0:
                        Seq2=Clo2Seq[Clo2]
                        DifC=self.CountDifNum(Seq1,Seq2)
                        if DifC==0:
                            Combo.append(Clo2.replace('#',''))
                            Done.append(Clo2)
                NewName=''.join(Combo)
                for C in Combo:
                    Clo2NewName[C]=NewName
                NewCloSeqDic['#'+NewName]=Seq1	
        CloBuild=self.UpMeg(NewCloSeqDic, [])
        self.save_mega_alignment_to_file(OutFileID+'.meg', CloBuild)
        ...
        for Tu in T2C2F:
            ...
            for C in C2F:
                NewC=Clo2NewName[C]
                if NewC not in NewC2F: NewC2F[NewC]=0
                CloFre=NewC2F[NewC]+C2F[C]
                if CloFre>1: CloFre=1	
                NewC2F[NewC]=CloFre	
            NewT2C2F[Tu]=NewC2F

        self.save_frequency_table_to_file(OutFileID+'.txt', NewT2C2F, [])			
